...
const VendorDetail = ({ navigation, route }) => {
  ...
  const deleteVendor = async (id) => {
    setLoading(true)
    try {
      const { message } = await apiCall.deleteVendor(id)
      setAlertType('s')
      ...
      <View style={{ paddingHorizontal: width * 0.032, flex: 1 }}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{
          ...
          <View style={{ paddingHorizontal: width * 0.02 }}>
            ...
            <View style={{ marginVertical: width * 0.07, flexDirection: 'row', gap: 8, justifyContent: 'center', marginTop: height * 0.5 }}>
              <TouchableOpacity disabled={loading} onPress={() => deleteVendor(updateData?.vendorID)} activeOpacity={1} style={{
                width: width * 0.13,
                height: width * 0.13,
              ...
              <TouchableOpacity onPress={() => navigation.navigate('updateVendor', { data: updateData })} activeOpacity={1} style={{
                width: width * 0.13,
                height: width * 0.13,
                marginRight: width * 0.02,
