...
const AddWalkinVisitor = ({navigation}) => {
  // console.log(navigation)
  const {control, handleSubmit, setValue, clearErrors} = useForm();

  const [appartmentModal, setAppartmentModal] = useState(false);
  ...
  console.log("🚀 ~ AddWalkinVisitor ~ appartmentId:", appartmentId)
  ...
  const submit = async data => {
    console.log('🚀 ~ file: index.js:107 ~ submit ~ data:', data);
    setLoading(true);
    try {
      const {
        CarNumber,
      ...
      let result = await apiCall.postWalkinVisitor(param);
      setShowSuccess(true);
      setTimeout(() => {
        goBack();
      }, 2000);
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: colors.white}}>
      ...
      <View style={{paddingHorizontal: width * 0.032, flex: 1}}>
        <Header onBack={goBack} title={'Add Walkin Visitor'} />
        ...
        <ScrollView
          ...
          <View style={{flex: 1, marginTop: width * 0.09}}>
            ...
            <DropDown
              ...
              img={Images.homeWithoutName}
              onPress={() => {
                setAppartmentModal(true);
              }}
            />
          ...
          <CustomModal
            data={appartmentData}
            modalVisible={appartmentModal}
            closeModal={() => setAppartmentModal(false)}
            getvalue={e => {
              setValue('appartmentId', e?.unit);
...
export default AddWalkinVisitor;
