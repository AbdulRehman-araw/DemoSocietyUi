import { StyleSheet, Text, View, Modal, TouchableOpacity, SafeAreaView, ScrollView, Dimensions, TextInput, Alert, Platform } from 'react-native'
...
const AdminModal = ({ visible, close, getData, navigation }) => {
    ...
    return (
        <View>
            ...
            <Modal visible={visible} animationType='fade' onRequestClose={() => close(false)}>

                <SafeAreaView style={[styles.mainContainer, { marginLeft: Platform.OS == "ios" ? 10 : 0 }]}>
                    <Header
                        onBack={() => close(false)}
                        title={"Add New"}
                        showRightBtn={false}
                    ...
                    <ScrollView contentContainerStyle={styles.cont} showsVerticalScrollIndicator={false}>
                        <View style={{
                            ...
                            <CustomText fontWeight={fontsFamily.bold} style={styles.topText}>E-Polling</CustomText>
                            <TextInput
                                placeholder='Title'
                            ...
                            <View style={{ width: '100%' }}>
                                <TimeDateBtn
                                    ...
                                    title={endTime == new Date() ? 'End Time' : getFormattedDate(endTime)}
                                    prefixIcon={true}
                                    setOpen={() => [setOpen(true), setMode('endTime')]}
                                />
                            </View>
                            {errorMessage ? <CustomText style={{ color: colors.danger, marginBottom: width * 0.02 }}>{errorMessage}</CustomText> : null}
